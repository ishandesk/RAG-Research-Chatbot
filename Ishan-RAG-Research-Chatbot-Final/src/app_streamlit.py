...
Description:
Retrieval-Augmented Generation chatbot for research-oriented document Q&A.
"""
...
def answer_template(language="english"):
    """Pass the standalone user_query along with the chat history and context
    to the `LLM` wihch will answer."""

    template = f"""Answer the user_query at the end, using only the following context (delimited by <context></context>).
...
def create_ConversationalRetrievalChain(
    ...
    First, it passes the follow-up user_query along with the chat history to an LLM which rephrases
    the user_query and generates a standalone query.
    This query is then sent to the retriever, which fetches relevant documents (context)
    and passes them along with the standalone user_query and chat history to an LLM to answer.
    """

    # 1. Define the standalone_question prompt.
    # Pass the follow-up user_query along with the chat history to the `condense_question_llm`
    # which rephrases the user_query and generates a standalone user_query.

    condense_question_prompt = PromptTemplate(
        input_variables=["chat_history", "user_query"],
        template="""Given the following conversation and a follow up user_query, 
rephrase the follow up user_query to be a standalone user_query, in its original language.\n\n
Chat History:\n{chat_history}\n
...
Standalone user_query:""",
    ...
    # Pass the standalone user_query + the chat history + the context (retrieved documents)
    # to the `LLM` wihch will answer
...
def get_response_from_LLM(prompt):
    """invoke the LLM, get llm_response, and display results (answer and source documents)."""
    try:
        # 1. Invoke LLM
        llm_response = st.session_state.chain.invoke({"user_query": prompt})
